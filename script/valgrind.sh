...
run_with_valgrind()
{
	valgrind --suppressions=$ROOT_DIR/resources/readline_suppression.supp --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=$VALGRIND_OUTPUT $ROOT_DIR/$TEST_EXEC > $ROOT_DIR/$TEST_RES
}
